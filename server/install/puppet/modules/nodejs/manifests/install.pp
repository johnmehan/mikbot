...
notify { "Install directory: ${::nodejs::params::install_dir}":}
notify {"Params: ${node_version} ":}
  ...
  exec { "node-install-${node_version}":
    ...
    user    => 'root',
    #unless  => "test -f ${::nodejs::params::install_dir}/ode-${node_version}/node",
    unless  => "test -L /usr/local/bin/node",
    timeout => 0,
